...
<div class="min-h-screen bg-gray-800 text-white flex flex-col items-center py-10 px-4">
    <form action="/samples/editor" method="post" class="main-container">
        @csrf
        <textarea name="editorData" id="editor">
			<p></p>
		</textarea>
        <p><input type="submit" value="Submit" style="color: white" class="bg-blue-700 p-1 rounded-md text-center"></p>
    </form>

    @foreach ($editorText as $text)
    {{ print_r($text->textData) }}
    @endforeach
    ...
    <div
        class="temporary-why-we flex items-center flex-col justify-center p-5 max-w-3xl w-full bg-gray-700 rounded-lg mb-8 text-center ">
        <h2>Огромный опыт</h2>
    ...
    <div class="firstVariant pb-15 max-w-3xl w-full bg-gray-700 p-8 rounded-lg mb-8 text-center">
        <form id="firstForm" action="/samples/1" method="POST">
            ...
            <div class="pswp-gallery flex flex-wrap gap-4 justify-center" id="gallery">
                ...
                <a href="{{ asset("images/samplePhotos/" . $photo["file_name"]) }}" data-pswp-width="{{ $photo['width'] }}" data-pswp-height="{{ $photo['height'] }}" class="block overflow-hidden rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
                <img src="{{ asset("images/samplePhotos/" . $photo["file_name"]) }}" class="w-48 h-32 object-cover object-center"></a>
                @endif
                @endforeach
            </div>

            <button type="submit"
                class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-4 transition duration-200 cursor-pointer">Save</button>
        </form>
    </div>
