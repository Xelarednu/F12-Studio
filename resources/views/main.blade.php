...
@section("content")
<div class="main-page container mx-auto min-w-full" id="top">
    ...
    <div class="first-screen-nav relative min-h-screen w-full">
        ...
        <header class="flex items-center justify-between text-white ml-50 mr-30 pt-5">
            <div class="logo-name flex items-center space-x-2">
                <img class="w-14 h-14 mr-6" src="{{ asset("images/F12 Logo 2.svg") }}" alt="logo">
                <p class="text-xl">F12 Studio</p>
            </div>

            <nav class="flex items-center space-x-4">
                <a href="/photoExamples" class="text-xl font-semibold">Примеры</a>
                <a href="/samples" class="text-xl font-semibold">Образцы</a>
                <a href="{{ route('client.login') }}" class="pr-24 text-xl font-semibold">Выбор фотографий</a>
                @if (session("admin_auth") == true)
                <a href="/dashboard" class="text-xl font-semibold">Админ панель</a>
                @else
                <a href="{{ route('admin.login') }}" class="text-xl font-semibold">Вход</a>
                @endif
            </nav>
        </header>

        <div class="first-screen text-white flex flex-row">
            <div class="social-media-links flex flex-col w-25 my-auto m-10">
                <div class="vertical-line rounded mx-auto"></div>
            ...
            <div class="sections flex flex-col mt-20">
                <section class="greetings mb-30">
                    <h1>Вас приветствует фотостудия F12 Studio</h1>
                    <p class="w-130 m-5">Наша главная цель проста: создавать качественные фотографии. Будь то мимолетный
                        момент, веха или суть того, кем вы являетесь. Мы здесь, чтобы превратить ваше видение в вечное
                        искусство.</p>
                    <a href="#services" class="ml-33 inline-block group">
                ...
                <section class="photo-choose-transition">
                    <h1>Переход на страницу выбора фотографий</h1>
                    <p class="w-125 m-5">Если вы уже сфотографировались и получили код от фотографа, то переходите на
                        страницу выбора фотографий по кнопке ниже.</p>
                    <a href="{{ route('client.login') }}" class="ml-33 inline-block group">
                        <div class="border-5 border-solid border-[#FFB85F] w-64 flex items-center justify-center backdrop-blur-xl bg-black/30 fill-button">
            ...
            <a href="#second_screen">
                <svg class="w-8 h-8 text-white absolute top-10/11 left-1/2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 14">
    ...
    <main>
        <div class="second-screen flex flex-col items-center text-center p-35 relative min-h-screen" id="second_screen">
            ...
            <div class="horizontal-line !w-70 !bg-black !m-0 absolute left-50 top-25 transform -translate-y-1/2"></div>
            <div class="horizontal-line !w-93 !bg-black !m-0 absolute right-50 top-235 transform -translate-y-1/2"></div>

            <div class="vertical-line !h-78 !bg-black !m-0 absolute right-50 top-85 transform -translate-y-1/2"></div>
            <div class="vertical-line !h-100 !bg-black !m-0 absolute left-30 top-150 transform -translate-y-1/2"></div>

            <div class="wrapper-container relative">
                <h1>Фотоальбомы у нас заказывают потому что:</h1>

                <div class="cards flex flex-wrap items-center text-center justify-center max-w-330">
                    <section class="huge-experience max-w-90 h-60 bg-gray-300/30 p-5 m-10 rounded-2xl">
                        <h2>Огромный опыт</h2>
                    ...
                    <section class="photo-choose max-w-90 h-65 bg-gray-300/30 p-5 m-10 rounded-2xl">
                    ...
                    <section class="own-studio max-w-90 h-65 bg-gray-300/30 p-5 m-10 rounded-2xl">
                        <h2>Своя студия</h2>
                    ...
                    <section class="main-goal max-w-120 h-50 bg-gray-300/30 p-5 m-10 rounded-2xl">
                        <h2 class="whitespace-nowrap">Главная цель - хорошие фотографии</h2>
                ...
                <h3 class="text-xl">Как выглядят выпускные альбомы можно посмотреть на странице образцов</h3>
        ...
        <div class="services-photos relative min-h-screen w-full pt-30 pb-30" id="services">
            ...
            <div class="services text-white flex flex-col items-center mb-25">
                ...
                <div class="services-cards flex flex-row justify-center flex-wrap text-black">
                    @foreach ($services as $service)
                        <section class="services-{{ $loop->iteration }} max-w-80 m-10 p-5 bg-gray-300/60">
            ...
            <div class="some-photos grid place-items-center gap-4 w-full px-4 text-white">
                <div class="line-photos flex items-center justify-center">
                ...
                <a href="/photoExamples" class="text-white bg-[#D66939] text-center px-6 py-3 rounded mt-4 max-w-2xl mx-auto">
                    Больше фотографий
                </a>
            </div>
        </div>
        ...
        <div class="send-us-message flex flex-col items-center relative min-h-200">
            ...
            <div class="line-message flex items-center justify-center pt-15">
                <h2 class="text-[#D66939] whitespace-nowrap ml-50 !mb-0">Отправить нам сообщение</h2>
                <div class="horizontal-line flex-grow !bg-[#D66939] ml-40 !mr-0"></div>
            </div>

            <form class="flex flex-col items-center justify-center gap-10 mt-24" action="/send-message" method="POST">
                @csrf
                <input type="email" name="email" placeholder="Е-майл" class="min-w-128 min-h-18" required />
                <textarea name="message" placeholder="Ваше сообщение" class="bg-white min-w-128 min-h-48" required></textarea>
                <button type="submit" class="bg-[#D66939] text-white p-3 min-w-38 cursor-pointer ">Отправить</button>
            </form>
        </div>

        <footer class="flex flex-row items-center justify-center gap-36 text-white bg-[#13131A] min-h-28">
            ...
            <div class="icons-number flex flex-wrap items-center justify-center gap-3 max-w-28">
            ...
            <div class="stuff flex flex-row gap-5">
                <p>Privacy policy</p>
                <p>Rights reserved</p>
            </div>
